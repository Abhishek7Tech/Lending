...
use anchor_spl::{
    associated_token::AssociatedToken, token_interface::{Mint,TransferChecked, TokenAccount, TokenInterface, transfer_checked}
};
...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    let price_updates = &mut ctx.accounts.price_update;
    ...
    match ctx.accounts.mint.to_account_info().key() {
        key if key == user.usdc_address => {
            ...
            let sol_price = price_updates.get_price_no_older_than(&Clock::get()?, MAXIMUM_AGE, &sol_feed_id)?;
        ...
        _ => {
            ...
            let usdc_price = price_updates.get_price_no_older_than(&Clock::get()?, MAXIMUM_AGE, &usdc_feed_id)?;
            let accured_interest = calculate_accrued_interest(user.deposited_sol, bank.interest_rate, user.last_updated)?;
            total_collateral = usdc_price.price as u64 * (user.deposited_usdc + accured_interest);
        }
    ...
    match ctx.accounts.mint.to_account_info().key()  {
        key if key == user.usdc_address => {
            ...
            user.borrowed_usdc_shares += borrowable_amount;
        },

        _ => {
            user.borrowed_sol += amount;
            user.borrowed_sol_shares += borrowable_amount;
        }
    }
