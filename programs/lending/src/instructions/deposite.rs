...
pub fn process_depoiste(ctx: Context<Deposite>, amount: u64) -> Result<()> {
    ...
    let deposite_ratio = amount.checked_div(bank.total_deposits).unwrap();
    let user_shares = bank.total_deposit_shares.checked_mul(deposite_ratio).unwrap();
