...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    match ctx.accounts.collateral_mint.to_account_info().key() {
        key if key == user.usdc_address => {
            let new_usd = calculate_accrued_interest(user.deposited_usdc, collateral_bank.interest_rate, user.last_updated)?;
            total_collateral =  usdc_price.price as u64 * new_usd;
        ...
        _ => {
            ...
            let new_usd = calculate_accrued_interest(user.borrowed_usdc, borrowed_bank.interest_rate, user.last_updated_borrow)?;
            total_borrowed = usdc_price.price as u64 * new_usd;
        }
    }
    ...
    if health_factor >= 1.0 {
        return  Err(ErrorCode::OverLiquidation.into());
    }

    let trasfer_to_bank = TransferChecked {
    ...
    let cpi_ctx = CpiContext::new(cpi_program.clone(), trasfer_to_bank);
    ...
    let cpi_ctx_for_liquidator = CpiContext::new(cpi_program.clone(), transfer_to_liquidator).with_signer(&signer_seeds);
    let liquidator_decimals = ctx.accounts.collateral_mint.decimals;
    transfer_checked(cpi_ctx_for_liquidator, liquidator_amount, liquidator_decimals)?;
...
fn calculate_accrued_interest(deposite: u64, interest_rate: u64, last_update: i64) -> Result<u64> {
    let current_time = Clock::get()?.unix_timestamp;
    let time_elapsed = current_time - last_update;
    let new_value = (deposite as f64 * E.powf(interest_rate as f64 * time_elapsed as f64)) as u64;
    Ok(new_value)
